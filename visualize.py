    ...
    def _render_map(self, tick):
        ...
        for node, (x, y) in NODE_POS.items():
            ...
            if is_intersection:
                ...
                ax.text(x + 0.04, y + 0.04, f">> {phase}",
                        fontsize=7.5, fontfamily=FONT,
                        color=phase_color, fontweight='bold',
                        ha='left', va='bottom', zorder=7)

        ea = self.model.emergency_agent
        if hasattr(ea, 'position') and ea.position in NODE_POS:
        ...
        for va in self.model.vehicle_agents:
            if hasattr(va, 'position') and va.position in NODE_POS:
    ...
    def _render_stats(self, tick):
        ...
        def row(label, value, color=WHITE):
            ...
            ax.text(0.02, y, label,
                    fontsize=7.5, fontfamily=FONT, color=DIM, va='top')
        ...
        ea = self.model.emergency_agent
        pos    = getattr(ea, 'position', 'inactive')
        active = getattr(ea, 'corridor_clear', False)
        row("Position",   pos)
